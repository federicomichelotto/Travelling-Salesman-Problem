...
void build_model(CPXENVptr env, CPXLPptr lp, instance *inst) {
    ...
    if (inst->param.solver == 0) { // optimal solver involved
        ...
        if (generate_path(path, "output", "model", optimal_model_name[inst->model_type], inst->param.name, inst->param.seed, "lp"))
            print_error("Unable to generate path");
...
//    for (int i = 0; i < inst->dimension; i++) printf("Node %d - (%f, %f) [%d]\n", i+1, node_list[i].x, node_list[i].y, node_list[i].flag);
    ...
    while (!done) {
        ...
        int selected_edge;                       // edge that corresponds to the smallest extra mileage to add the node k to the circuit
        int selected_node;
        ...
        edge_list[selected_edge].dist = dist(edge_list[selected_edge].prev, selected_node, inst); // Replace edge (i, j) with edge (i, k)
        edge_list[selected_edge].next = selected_node;

        node_list[selected_node].flag = 1; // Update nodes in circuit

        obj += min_value; // Update objective value

        if (iter == inst->dimension - 2) done = 1; // Because after n-2 iteration there are no more free edges

    }

    printf("Best objective value for starting node %d: %f\n", starting_node + 1, obj);

//    for (int i = 0; i < inst->dimension; i++) printf("Edge %d - (%d, %d) [%d]\n", i+1, edge_list[i].next, edge_list[i].prev, edge_list[i].flag);
...
double extra_mileage(instance *inst, int starting_node) {
    ...
    while (!done) {
        ...
        for (int i = 0;
             i < inst->dimension; i++) { // For every edge in the circuit find the one with minimum extra mileage
...
//    }
    ...
    if (random_number <= 0.5 ||
        count < 3) { // GRASP: 50% of the times (if there are sufficient nodes out of the circuit)
