...
int TSPopt(instance *inst)
{
    ...
    inst->u = (int *)calloc(inst->nodes, sizeof(int));
    inst->v = (int *)calloc(inst->nodes, sizeof(int));
    inst->n_edges = 0; // count edges
    for (int i = 0; i < inst->nodes; i++)
    {
        for (int j = i + 1; j < inst->nodes; j++)
        {
            ...
            {
                printf("  ... x(%3d,%3d) = 1\n", i + 1, j + 1);
                inst->u[inst->n_edges] = i;
                inst->v[inst->n_edges] = j;
                if (++inst->n_edges > inst->nodes) print_error("more edges than nodes, not a tour.");
            }
        }
    }

    if (inst->n_edges != inst->nodes)
