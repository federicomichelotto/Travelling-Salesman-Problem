...
{
    char *commandsForGnuplot[] = {"set title \"Solution plot\"", "unset key", "set xrange [0:*]", "set yrange [0 :*]", "plot \"data.temp\" with lines"};
    ...
    for (int i = 0; i < num_edges; i++)
    {
        ...
        fprintf(temp, "%lf %lf \n%lf %lf \n\n", x[u[i]], y[u[i]], x[v[i]], y[v[i]]); // write coords of the two nodes
    }
    fclose(temp);
